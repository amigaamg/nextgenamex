...
// PAYHERO STK PUSH
// ═══════════════════════════════════════════════════════════════════════════
async function initiateSTK(phone: string, amount: number, appointmentId: string, patientName: string, specialty: string) {
  const creds = btoa(`${process.env.NEXT_PUBLIC_PAYHERO_USERNAME}:${process.env.NEXT_PUBLIC_PAYHERO_PASSWORD}`);
  const res = await fetch('https://backend.payhero.co.ke/api/v2/payments', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Basic ${creds}` },
    body: JSON.stringify({
      amount, phone_number: phone,
      channel_id: process.env.NEXT_PUBLIC_PAYHERO_CHANNEL_ID,
      provider: 'm-pesa', external_reference: appointmentId,
...
function BookModal({ svc, patient, onClose }: { svc: Service; patient: Patient; onClose: () => void }) {
  ...
  const handlePay = async (targetApptId?: string) => {
    ...
    setLoading(true); setError('');
    try {
      const data = await initiateSTK(phone, svc.price, id, patient.name, svc.specialty);
      await updateDoc(doc(db, 'appointments', id), {
        paymentRef: data.reference || data.CheckoutRequestID || '',
        paymentStatus: 'processing', phone,
      });
      setStep('done');
    } catch (e: any) {
      setError(e.message);
      // Allow retry — keep existing appointment
...
function PaymentsPanel({ appointments, patient }: { appointments: Appointment[]; patient: Patient }) {
  ...
  const retryPay = async (appt: Appointment) => {
    if (!phone.match(/^(07|01|\+2547|\+2541)\d{8}$/)) { setPayMsg('Enter valid Safaricom number'); return; }
    setPaying(appt.id); setPayMsg('');
    try {
      const data = await initiateSTK(phone, appt.amount || 0, appt.id, patient.name, appt.specialty || 'Consultation');
      await updateDoc(doc(db, 'appointments', appt.id), {
        paymentRef: data.reference || '', paymentStatus: 'processing', phone,
      });
      setPayMsg('STK push sent! Check your phone.');
    } catch (e: any) { setPayMsg('Error: ' + e.message); }
    setPaying(null);
  };
